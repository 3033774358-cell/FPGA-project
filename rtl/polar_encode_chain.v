...
module polar_encode_chain #(
    ...
    // R=1(码率1, MCS8/12): 无信道编码, 码块 = (消息‖CRC) 直传, 不做极化变换。
    // 附录H符合性: TV313 类头段直传(1000msg+CRC24B)、尾段直传(144msg+CRC24B), 块长=信息长。
    // 实现: 冻结掩码退化为"前 Kblk 位信息、其余置0"(用位置计数 acnt 比较, 无需真冻结集/变址);
    //       蝶形旁路(blk_out 取 u_bottom)、级联长度 elen 取 Kblk。 R<1 时全部走原路(rate1=0)。
    wire        rate1  = (r16 == 5'd16);
    wire        cur_fz = rate1 ? (acnt >= Kblk) : fz_sh[0];
    ...
    // R=1 旁路: 直接取装配好的 u_bottom(= 直传的 V 序列, 低 Kblk 位), 长度 Kblk; 否则取蝶形输出, 长度 N
    wire [NMAX-1:0] blk_out = rate1 ? u_bottom : bf_d;
    wire [10:0]     elen    = rate1 ? Kblk : (11'd1 << nlog_blk);   // E_r = R=1时信息长, 否则 N
    code_block_concat #(.NMAX(NMAX)) u_cat (
        .clk(clk), .rst_n(rst_n),
        .s_load(cat_load), .s_blk(blk_out), .s_elen(elen), .s_last(cat_last),
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            case (state)
                ...
                SA_INIT: begin
                    c1024<=0; km<=B_l;
                    // ---- 类型2 + R=1: 整帧直通, 不进表20/24 查表 ----
                    // 附录H TV213/214/215 金标准为 "P:B" (单块直传, 长度=B)。
                    // R=1 = 不做信道编码, 本就不该按表分块; 且【表24】无 R=1 条目
                    // (仅 R=5/8,3/4,7/8), 若误入查表会得到 K=0 的空块 -> 卡死。
                    // 限制: 直通块占用单个 u_sh(NMAX位), 故要求 B<=NMAX(=1024);
                    //       附录H 该组合最大 B=1016, 满足。
                    if (r16 == 5'd16) begin
                        padv<=0; Kblk<=B_l[10:0]; Kmsg_cur<=B_l[10:0];
                        crc_on<=0; vpos<=0;          // 类型2 无码块CRC
                        nlog_blk<=4'd10; blk_is_last<=1;
                        c512<=0; c256<=0; c128<=0; c64<=0;  // 清残留, 保证 S_ROUTE 直接收尾
                        fg_start<=1; state<=S_FZ;    // rate1 下 cur_fz 用 acnt 比较, 冻结集不参与
                ...
                SA_PAD: begin                        // 末块不足位: K=实际消息位(不补零)
                    // 附录H符合性: 类型2末块消息不足时, 不补零到表24-K, 而是收缩 K=消息位数
                    // (对完附录H数据: 类型2末块金标准为 (N, Kmsg) 无pad; 类型3/4则仍前补0)
