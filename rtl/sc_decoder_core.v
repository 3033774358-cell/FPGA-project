...
module sc_decoder_core #(
    ...
    localparam [2:0] ST_ROM_FILL  = 3'd3;
    localparam [2:0] ST_READY     = 3'd4;
    localparam [2:0] ST_ACTIVE    = 3'd5;
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            case (state)
                ...
                // -------------------------------------------------------------
                // Fast-SSC：等待节点类型 ROM 填充完成
                // -------------------------------------------------------------
                ST_ROM_FILL: begin
                    config_busy <= 1'b1;

                    if (fast_rom_done) begin
                        config_busy <= 1'b0;
                        config_done <= 1'b1;
                        state       <= ST_READY;
                    end
