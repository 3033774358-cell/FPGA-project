...
module sc_uhat_mem #(
    ...
    // =========================================================================
    // uhat_mem 写口: **独立无复位块** (与 sc_llr_mem / sc_beta_mem 同一处理)
    // -------------------------------------------------------------------------
    // 原写语句在带异步复位的 always 块内, 导致 Vivado:
    //   [Synth 8-4767] 无法推断 BRAM/DRAM, 退化成 1024 个触发器;
    //   [Synth 8-7137] "uhat_mem_reg 的 Set 与 reset 同优先级" —— 仿真/综合行为
    //                  可能不一致(真实风险, 非风格告警)。
    // 移出后语义完全不变: 写使能 = 原来的 (!output_busy && wr_en), 读口仍为异步读
    // (assign), 故可推断为分布式 RAM(LUTRAM, 支持异步读), 省下约 1024 个触发器。
    // =========================================================================
    always @(posedge clk) begin
        if (!output_busy && wr_en)
            uhat_mem[wr_index] <= wr_bit;
    end
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            if (!output_busy) begin
                ...
                // (存储器写已移到下方**独立无复位块** —— 见 uhat_mem 写口说明)
