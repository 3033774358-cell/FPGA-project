...
// =============================================================================
// sc_bc_n1024_system_tb.v
// N=1024、K=512，正式sc_controller + 现有sc_datapath系统级无噪声环回测试。
// 编码：d=u*F^(x10)，无bit reversal；d=0映射+40，d=1映射-40。
// frozen[i]=1表示u[i]冻结为0。NMAX仍固定1024。
// =============================================================================
module sc_bc_n1024_system_tb;

    parameter integer NMAX      = 1024;
    parameter integer N         = 1024;
    parameter integer K         = 512;
    parameter integer N_LOG     = 10;
    ...
    parameter integer TIMEOUT_CYCLES = 80000;

    // N=1024、K=512固定冻结掩码；bit i对应u[i]。
    // 由3GPP TS 38.212表5.3.1.2-1的1024项可靠度序列生成：
    // 最不可靠的512个位置冻结，最可靠的512个位置作为信息位。
    localparam [N-1:0] FROZEN_N1024_K512 = {
        128'h00000000000000000000000000000001,
        128'h0000000000000001000000070117177F,
        128'h00000000000000170001011701173FFF,
        128'h0001011F077F7FFF177F7FFF7FFFFFFF,
        128'h00000000000101170001013F077F7FFF,
        128'h0003077F177F7FFF17FFFFFFFFFFFFFF,
        128'h011717FF1FFFFFFF7FFFFFFFFFFFFFFF,
    ...
    initial begin
        ...
        $display("============================================================");
        $display("sc_bc_n1024_system_tb completed");
        $display("Total cases  = %0d", case_count);
        $display("Total checks = %0d", check_count);
        $display("Error count  = %0d", error_count);

        if (error_count == 0) begin
            $display("RESULT: sc_bc_n1024_system_tb PASS");
        end
        else begin
            $display("RESULT: sc_bc_n1024_system_tb FAIL");
        end
    ...
    initial begin
        ...
        $display("============================================================");
        $display("RESULT: sc_bc_n1024_system_tb GLOBAL TIMEOUT");
        $display("============================================================");
